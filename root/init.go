...
func InitWorkspace(server_alias, workspace_password string) {
	...
	zip_destination_path := filepath.Join(workspace_path, ".PKr", "Files", "Current", "")
	...
	// Storing Key
	err = os.WriteFile(filepath.Join(zip_destination_path,"AES_KEY"), key, 0644)
	if err != nil {
	...
	// Storing IV
	err = os.WriteFile(filepath.Join(zip_destination_path, "AES_IV"), iv, 0644)
	if err != nil {
	...
	zipped_filepath := filepath.Join(zip_destination_path, hash_zipfile+".zip")
